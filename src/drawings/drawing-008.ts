...
const symbolB = (
  ...
  ctx.lineWidth = size * scale * .5
  ctx.beginPath()
  ctx.moveTo(x - offset - Math.ceil(Math.random() * 10), y - offset - Math.ceil(Math.random() * 10))
  ctx.lineTo(x + offset + Math.ceil(Math.random() * 10), y + offset + Math.ceil(Math.random() * 10))
  ctx.moveTo(x + offset + Math.ceil(Math.random() * 10), y - offset - Math.ceil(Math.random() * 10))
  ctx.lineTo(x - offset - Math.ceil(Math.random() * 10), y + offset + Math.ceil(Math.random() * 10))
...
const drawing: DrawScriptType = async (ctx, [sizeX, sizeY]) => {
  ...
  points.forEach(([x, y]: [number, number]) => {
    symbolA(ctx, Math.floor(x), Math.floor(y), dispPoints[Math.floor(x) + sizeX * Math.floor(y)])
    symbolB(ctx, Math.floor(x), Math.floor(y), dispPoints[Math.floor(x) + sizeX * Math.floor(y)])
  })
